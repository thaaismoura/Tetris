(() => {
  ...
  // Canvas: usar os atributos width/height fixos (300x600) e CSS com aspect-ratio
  // -> não alteramos tamanhos via JS para evitar "piscadas" no mobile.
  ...
  // Bag 7
  let bag=[]; function refillBag(){ bag=Object.keys(SHAPES).sort(()=>Math.random()-0.5); }
  ...
  function spawn(){ cur=newPiece(next.type); next=newPiece(nextType()); if(collide(board,cur)){ running=false; showOverlay('Reiniciar'); } drawNext(); }
  function showOverlay(btnText='Começar'){ overlay.style.display='grid'; overlay.querySelector('.overlay-content').innerHTML = `<button id="btnOverlay" class="primary">${btnText}</button>`; const b=document.getElementById('btnOverlay'); b.onclick=()=>{ overlay.style.display='none'; startGame(); }; }
  ...
  function rotateCur(){ const r=rotate(cur.shape); const t={...cur,shape:r}; if(!collide(board,t)) cur.shape=r; else if(!collide(board,{...t,x:cur.x-1})) {cur.x-=1; cur.shape=r;} else if(!collide(board,{...t,x:cur.x+1})) {cur.x+=1; cur.shape=r;} }
  ...
  function startGame(){ reset(); running=true; paused=false; lastTime=performance.now(); requestAnimationFrame(animate); }

  // Botão da tela inicial (só um botão verde)
  if(btnStart){ btnStart.addEventListener('click', ()=>{ overlay.style.display='none'; startGame(); }); }

  // Controles por toque: pointerdown garante iOS
  ...
  document.querySelectorAll('.ctrl').forEach(btn=>{ const a=btn.dataset.action; bindPointer(btn, ()=>handle(a)); });
  function handle(a){ if(!running && a!=='music' && a!=='pause'){ startGame(); } if(a==='left') move(-1); else if(a==='right') move(1); else if(a==='rotate') rotateCur(); else if(a==='soft') softDrop(); else if(a==='hard') hardDrop(); else if(a==='pause') togglePause(); else if(a==='music') toggleMusic(); }
  ...
  // Exibir overlay inicial com apenas um botão
