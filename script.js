(() => {
  ...
  function resizeCanvas(){
    const width = canvas.clientWidth * 0.9; // reduz um pouco mais a área
  ...
  document.querySelectorAll('.ctrl').forEach(btn=>{btn.addEventListener('click',()=>handle(btn.dataset.action));btn.addEventListener('touchstart',e=>{e.preventDefault();handle(btn.dataset.action)},{passive:false});});
